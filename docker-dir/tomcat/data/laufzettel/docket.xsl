...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:goobi="http://www.goobi.org/logfile" xmlns:fn="http://www.w3.org/2005/xpath-functions" exclude-result-prefixes="fo">
						  ...
						  <fo:table line-height="14pt">
							...
							<fo:table-body>
								<fo:table-row border-width="1pt" border-style="solid">
									<fo:table-cell>			
											<fo:block></fo:block>		
										<xsl:for-each select="goobi:comments/goobi:comment">
											<fo:block>		

<!--											
	<xsl:variable name="com">
													<xsl:value-of select="fn:replace(., '\d\d\.\d\d\.\d\d\d\d\s\d\d:\d\d:\d\d:\s', '')" />
												</xsl:variable>
												<xsl:value-of select="fn:replace($com, '\(\w+,\s\w+\)', '')" /> -->
												<xsl:value-of select="fn:replace(., '\d\d\.\d\d\.\d\d\d\d\s\d\d:\d\d:\d\d:\s', '')" />
											</fo:block>
										</xsl:for-each>
									</fo:table-cell>			
